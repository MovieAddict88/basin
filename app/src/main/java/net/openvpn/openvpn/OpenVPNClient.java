...
public class OpenVPNClient extends OpenVPNClientBase implements OnClickListener, OnTouchListener, OnItemSelectedListener, OnEditorActionListener {
    ...
    private static final String SELECTED_PROMO_ID_KEY = "selected_promo_id";
    ...
    private void ui_setup(boolean active, int flags, String profile_override) {
        ...
        if (!((UIF_RESET & flags) == 0 && orig_active == this.last_active)) {
            ...
            if (proflist == null || proflist.size() <= 0) {
                ...
                this.promo_spinner.setEnabled(!active);
            }
            if (prof != null) {
    ...
    public void onItemSelected(AdapterView<?> parent, View v, int position, long id) {
        ...
        if (viewid == R.id.profile) {
            if (!is_active()) {
                ...
                ui_setup(RETAIN_AUTH, UIF_RESET | UIF_PROFILE_SETTING_FROM_SPINNER, null);
            }
            ...
            if (!is_active()) {
                ...
                if (selectedPromo != null) {
                    // Save the selected promo ID to SharedPreferences
                    prefs.set_string(SELECTED_PROMO_ID_KEY, String.valueOf(selectedPromo.getId()));
                }
                mBoundService.refresh_profile_list(selectedPromo != null ? selectedPromo.getId() : null);
            }
        }
    ...
    private class FetchPromosTask extends AsyncTask<Void, Void, ArrayList<Promo>> {
        private static final String PROMO_CACHE_KEY = "promo_cache";
        ...
        protected ArrayList<Promo> doInBackground(Void... voids) {
            try {
                ...
                try {
                    ...
                    String cachedJson = CacheUtil.loadFromCache(OpenVPNClient.this, PROMO_CACHE_KEY);
                    if (rawJson.equals(cachedJson)) {
                        return null; // Data is unchanged, no need to update UI
                    }

                    CacheUtil.saveToCache(OpenVPNClient.this, PROMO_CACHE_KEY, rawJson);
                    return parsePromos(rawJson);
                } finally {
                    urlConnection.disconnect();
                }
        ...
        protected void onPostExecute(ArrayList<Promo> promos) {
            if (promos != null && !promos.isEmpty()) {
                updatePromoSpinner(promos);
            }
        }

        private void loadPromosFromCache() {
            String promosJson = CacheUtil.loadFromCache(OpenVPNClient.this, PROMO_CACHE_KEY);
            if (promosJson != null) {
                ArrayList<Promo> promos = parsePromos(promosJson);
                if (promos != null) {
                    updatePromoSpinner(promos);
                }
            }
        ...
        private ArrayList<Promo> parsePromos(String json) {
            try {
                ...
                Log.e("FetchPromosTask", "Error parsing promos", e);
            }
            return null;
        }

        private void updatePromoSpinner(ArrayList<Promo> promos) {
            PromoAdapter promoAdapter = new PromoAdapter(OpenVPNClient.this, promos);
            promo_spinner.setAdapter(promoAdapter);

            // Restore the selected promo
            String selectedPromoId = prefs.get_string(SELECTED_PROMO_ID_KEY);
            if (selectedPromoId != null) {
                for (int i = 0; i < promos.size(); i++) {
                    if (String.valueOf(promos.get(i).getId()).equals(selectedPromoId)) {
                        promo_spinner.setSelection(i);
                        break;
                    }
                }
            }
