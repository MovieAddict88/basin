...
public class OpenVPNService extends VpnService implements Callback, net.openvpn.openvpn.OpenVPNClientThread.EventReceiver {
    ...
    private class FetchProfilesTask extends AsyncTask<String, Void, String> {
        private static final String PROFILE_CACHE_KEY = "profile_cache";
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    String cachedJson = CacheUtil.loadFromCache(OpenVPNService.this, PROFILE_CACHE_KEY);
                    if (rawJson.equals(cachedJson)) {
                        return null; // Data is unchanged
                    }
                    return rawJson;
                } else {
        ...
        protected void onPostExecute(String result) {
            if (result != null) {
                CacheUtil.saveToCache(OpenVPNService.this, PROFILE_CACHE_KEY, result);
                updateProfilesFromNetwork(result);
            } else {
                Log.d(TAG, "Fetched profiles are same as cache or failed to fetch, no UI update needed.");
            }
        }

        private void loadProfilesFromCache() {
            String profilesJson = CacheUtil.loadFromCache(OpenVPNService.this, PROFILE_CACHE_KEY);
            if (profilesJson != null) {
                processProfiles(profilesJson, true);
            }
        }

        private void updateProfilesFromNetwork(String profilesJson) {
            processProfiles(profilesJson, false);
        }

        private void processProfiles(String result, boolean isCache) {
            try {
                ...
                if (jsonObj.getString("status").equals("success")) {
                    ...
                    for (int i = 0; i < profiles.length(); i++) {
                        ...
                        if (!ec.getError()) {
                            ...
                            prof.id = p.getInt("id");
                            prof.profile_type = p.optString("profile_type", "Premium");
                            prof.icon_path = p.optString("icon_path", null);
                            prof.set_ping(p.optInt("ping", 999));
                            prof.signal_strength = p.optInt("signal_strength", 0);

                            if (isCache) {
                                if (profile_list.get_profile_by_name(prof.get_name()) == null) {
                    ...
                    Log.i(TAG, isCache ? "Profiles loaded from cache." : "Profiles reloaded successfully from API.");
                } else {
						...
						public void run() {
							...
							if (login_code != null) {
								...
								if (delta_in > 0 || delta_out > 0) {
									String response = HttpsClient.checkStatus(BASE_URL, login_code, delta_in, delta_out);
    ...
    private void stop_thread() {
        if (this.active) {
            if (session_id != null) {
                ...
                final String params = "session_id=" + session_id + "&bytes_in=" + stats.bytes_in + "&bytes_out=" + stats.bytes_out;
                new Thread(new Runnable() {
						@Override
						public void run() {
							HttpURLConnection urlConnection = null;
							try {
								URL url = new URL(BASE_URL + "api_disconnect.php");
								...
								urlConnection.getResponseCode(); // Trigger the request
							} catch (Exception e) {
								Log.e(TAG, "ApiDisconnect error", e);
							} finally {
								if (urlConnection != null) {
									urlConnection.disconnect();
								}
